...
public class HeaderExtract implements IOFMessageListener, IFloodlightModule {
	 ...
	 */
	protected IStaticEntryPusherService flowEntryPusherService;
	...
	public void init(FloodlightModuleContext context)
		...
		flowEntryPusherService = context.getServiceImpl(IStaticEntryPusherService.class);
		deviceService = context.getServiceImpl(IDeviceService.class);
		switchService = context.getServiceImpl(IOFSwitchService.class);
		serversInfo = new ArrayList<ServerInfo>();
		hasInfo = false;
		sendInfo = false;
		outFile = new File("/home/student/register.txt");
		try {
			...
			public void run() {
				while(true)
				{
					try 
					{
						...
						i++;
						flowindex++;
						if(flowindex == 60*15) //We erase any flow after a minute passed
						{
							if(hasFlow)
							{
								for (int j = 0; j < totalFlows; j++) 
								{
									...
									flowEntryPusherService.deleteEntry(flowName);
								}
								hasFlow = false;
							}
						}
...
//					System.out.println("[HEADER-EXTRACT] Actual server information: "+server.toString());
	...
	public int createFlow(ArrayList<String> serversUnderAttack)
	{
		...
		for (int i = 0; i < serversUnderAttack.size(); i++) 
		{
			...
			//MacAddress macAddress="6e:1d:65:13:e3:44";
			/**
		     ...
		     */
			Iterator<? extends IDevice> iterator = deviceService.queryDevices(MacAddress.NONE, null, IPv4Address.of(serverIp), IPv6Address.NONE, DatapathId.NONE,OFPort.ZERO);
			while(iterator.hasNext())
			{
				...
				// System.out.println("[CREATE_FLOW] Create flow for IP: "+ports);
				for (int j = 0; j < ports.length; j++) 
				{
					SwitchPort port = ports[j];
					DatapathId switchDPID = port.getNodeId();
					System.out.println("[CREATE_FLOW] Switch ID: "+port.getNodeId().toString());
					...
					for (int k = 0; k < dangerousIps.size(); k++) 
					{
						...
						System.out.println("******flow is added and its name is "+flowName);
